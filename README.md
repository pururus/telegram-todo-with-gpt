...
Сейчас все модули уже работают, но все еще есть некоторый моменты, требующие доработки.
...
Также есть возмжнсть указывать, какой тип запроса ты отправляешь, через соответсвующие кнопки.

## Что нужно сделать
   - Добавить документация ко всем функциям, методам, классам
   - Почистить код от условных конструкций и тд.
   - Улучшить парсер, тк сейчас из-за рандома в ГПТ может неправильно считываться дата и время
   - Улучшить UI экспириенс. Добавить разные сообщения об ошибках и тд
   - Добавить возможность сменить идентификатор календаря или тудуиста
   - Добавить и отдебажить асинки
   - Добавить проверку на валидность идентификаторов
